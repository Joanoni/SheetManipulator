...
| 035| 2026-03-16 | StressTest: In-process TestClient (no live server) | Stress tests use FastAPI TestClient in-process to avoid requiring a running server, keeping tests self-contained and portable. | Approved |
| 036| 2026-03-16 | StressTest: All 50 concurrent CSV POSTs succeeded | Under 50-thread concurrency, the FileLock's O_CREAT|O_EXCL mechanism serialised all writes without data loss or lock timeouts; CSV file integrity confirmed. | Approved |